...
function prompt_git_time_since_commit() {
  if git rev-parse --git-dir > /dev/null 2>&1; then
    if [[ $(git log 2>&1 > /dev/null | grep -c "^fatal: bad default revision") == 0 ]]; then
      ...
      if [ "$HOURS" -gt 24 ]; then
          "$1_prompt_segment" "$0" "$2" "${COLOR_BACKGROUND}" "${COLOR_TEXT}" "${DAYS}d | $(git_prompt_short_sha)" "TIME_ICON"
      elif [ "$MINUTES" -gt 60 ]; then
          "$1_prompt_segment" "$0" "$2" "${COLOR_BACKGROUND}" "${COLOR_TEXT}" "${HOURS}h${SUB_MINUTES}m  | $(git_prompt_short_sha)" "TIME_ICON"
      else
          "$1_prompt_segment" "$0" "$2" "${COLOR_BACKGROUND}" "${COLOR_TEXT}" "${MINUTES}m | $(git_prompt_short_sha)" "TIME_ICON"
      fi
    else
...
########################################################################
prompt_git_prompt_short_sha() {
"$1_prompt_segment" "$0" "$2" "green" "black" "" "TIME_ICON"
}
